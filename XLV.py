...
try:
    ...
    # Get daily data for XLV
    # outputsize can be 'compact' (last 100 data points) or 'full' (complete history)
    data, meta_data = ts.get_daily(symbol='XLV', outputsize='full')
    ...
    plt.title('XLV Stock Price with Technical Indicators')
    plt.xlabel('Date')
    ...
    # Save to CSV
    df.to_csv('xlv_data.csv')
    print("\nData has been saved to 'xlv_data.csv'")
